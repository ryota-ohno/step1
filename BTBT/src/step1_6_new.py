...
def main_process(args):
    ...
    auto_csv_path = os.path.join(auto_dir,'step1.csv')
    if not os.path.exists(auto_csv_path):        
        df_E = pd.DataFrame(columns = ['A2','a','b','theta','E','E_t','E_p','machine_type','status','file_name'])##いじる
...
def get_opt_params_dict(df_cur, init_params_dict,fixed_params_dict):
    ...
    A2_init_prev = init_params_dict['A2']
    while True:
        E_list=[];A2_list=[]
        for A2 in [A2_init_prev-1,A2_init_prev,A2_init_prev+1]:
            ...
            if len(df_val_ab)==0:
                return False,{'A2':A2}
            A2_list.append(A2);E_list.append(df_val_ab['E'].values[0])
        A2_init = A2_list[np.argmin(np.array(E_list))]
        if A2_init==A2_init_prev:
            return True,{'A2':A2_init}
        else:
