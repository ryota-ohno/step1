...
def get_monomer_xyzR(monomer_name,Ta,Tb,Tc,A1,A2,A3,phi=0.0,isFF=False):
    ...
    df_mono=pd.read_csv('~/Working/step1/{}/assets/qe_1_center.csv'.format(monomer_name))
...
def get_monomer_xyzR_(monomer_name,Ta,Tb,Tc,A1,A2,A3,phi=0.0,isFF=False):
    T_vec = np.array([Ta,Tb,Tc])
    df_mono=pd.read_csv('~/Working/step1/{}/assets/qe_2_center.csv'.format(monomer_name))
    atoms_array_xyzR=df_mono[['X','Y','Z','R']].values
...
def make_gjf_xyz(auto_dir,monomer_name,params_dict,machine_type,isInterlayer):
    ...
    R4 = 0#params_dict['R4']
    R5=2*R3-R4
    phi1 = 0#params_dict.get('phi1',0.0)
    phi2 = 0#-params_dict.get('phi2',0.0)##入力ファイルにおけるphi1とphi2の符号は一致させる
    print(phi1, phi2)
    ...
    if b_>a_:
        ...
        monomer_array_p2 = get_monomer_xyzR_(monomer_name,a_,0,R5,-A1,A2,-A3, phi2)
        ...
        monomer_array_p2 = get_monomer_xyzR_(monomer_name,0,b_,R4,-A1,A2,-A3, phi2)
