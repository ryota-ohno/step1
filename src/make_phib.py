...
def get_monomer_xyzR(monomer_name,Ta,Tb,Tc,A1,A2,A3,phi_r,phi_b,isFF=False):
    ...
    df_mono=pd.read_csv('~/Working/alkyl_chain/monomer/{}/monomer.csv'.format(monomer_name))
...
def make_gaussview_xyz(auto_dir,monomer_name,params_dict,isInterlayer=False):
    ...
    phi1 = params_dict.get('phi1',0.0); phi2 = params_dict.get('phi2',0.0); phib = params_dict.get('phib',0.0)
    print(phi1, phi2)
    a =np.array([a_,0,0])
    b =np.array([0,b_,0])
    
    monomer_array_i = get_monomer_xyzR(monomer_name,0,0,0,A1,A2,A3, phi1,phib)
    if a_>b_:
        monomer_array_p1 = get_monomer_xyzR(monomer_name,0,b_,0,A1,A2,A3, phi1,phib)
        monomer_array_p2 = get_monomer_xyzR(monomer_name,0,-b_,0,A1,A2,A3, phi1,phib)
...
#         monomer_array_ip2 = get_monomer_xyzR(monomer_name,c[0],c[1]-b_,c[2],A1,A2,A3)
        ...
        monomer_array_p2 = get_monomer_xyzR(monomer_name,-a_,0,0,A1,A2,A3, phi1,phib)
...
#     monomer_array_i0 = get_monomer_xyzR(monomer_name,c[0],c[1],c[2],A1,A2,A3)
    monomer_array_t1 = get_monomer_xyzR(monomer_name,a_/2,b_/2,0,-A1,A2,-A3, phi2,phib)
    monomer_array_t2 = get_monomer_xyzR(monomer_name,a_/2,-b_/2,0,-A1,A2,-A3, phi2,phib)
    monomer_array_t3 = get_monomer_xyzR(monomer_name,-a_/2,-b_/2,0,-A1,A2,-A3, phi2,phib)
    monomer_array_t4 = get_monomer_xyzR(monomer_name,-a_/2,b_/2,0,-A1,A2,-A3, phi2,phib)
...
def make_gjf_xyz(auto_dir,monomer_name,params_dict,machine_type,isInterlayer):
    a_ = params_dict['a']; b_ = params_dict['b']; c = np.array([params_dict['cx'],params_dict['cy'],params_dict['cz']])
    A1 = params_dict['A1']; A2 = params_dict['A2']; A3 = params_dict['theta']
    phi1 = params_dict.get('phi1',0.0); phi2 = -phi1; phib = params_dict.get('phib',0.0)
    print(phi1, phi2)
    ...
    monomer_array_i0 = get_monomer_xyzR(monomer_name,c[0],c[1],c[2],A1,A2,A3, phi1,phib)
    monomer_array_t1 = get_monomer_xyzR(monomer_name,a_/2,b_/2,0,-A1,A2,-A3, phi2,phib)
    monomer_array_t2 = get_monomer_xyzR(monomer_name,a_/2,-b_/2,0,-A1,A2,-A3, phi2,phib)
    monomer_array_t3 = get_monomer_xyzR(monomer_name,-a_/2,-b_/2,0,-A1,A2,-A3, phi2,phib)
    monomer_array_t4 = get_monomer_xyzR(monomer_name,-a_/2,b_/2,0,-A1,A2,-A3, phi2,phib)
    monomer_array_it1 = get_monomer_xyzR(monomer_name,c[0]+a_/2,c[1]+b_/2,c[2],-A1,A2,-A3, phi2,phib)
    monomer_array_it2 = get_monomer_xyzR(monomer_name,c[0]+a_/2,c[1]-b_/2,c[2],-A1,A2,-A3, phi2,phib)
    monomer_array_it3 = get_monomer_xyzR(monomer_name,c[0]-a_/2,c[1]-b_/2,c[2],-A1,A2,-A3, phi2,phib)
    monomer_array_it4 = get_monomer_xyzR(monomer_name,c[0]-a_/2,c[1]+b_/2,c[2],-A1,A2,-A3, phi2,phib)
...
def get_file_name_from_dict(monomer_name,paras_dict):
    ...
    for key,val in paras_dict.items():
        if key in ['a','b','cx','cy','cz','theta']:
            val = np.round(val,2)
        elif key in ['A1','A2','phi1','phib']:#,'theta']:
            val = int(val)
