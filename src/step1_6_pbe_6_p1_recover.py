...
from make_pbebj_p1_3_recover import exec_gjf##計算した点のxyzfileを出す
...
def main_process(args):
    ...
    auto_csv_path = os.path.join(auto_dir,'step1.csv')
    if not os.path.exists(auto_csv_path):        
        df_E = pd.DataFrame(columns = ['a','b','theta','R3','R4','phi1','phi2','E','E_p2','machine_type','status','file_name'])##いじる
...
def listen(auto_dir,monomer_name,num_nodes,isTest):##args自体を引数に取るか中身をばらして取るかの違い
    ...
    for idx,row in zip(df_queue.index,df_queue.values):
        ...
        if E_len !=5:##get Eの長さは計算した分子の数
            continue
        else:
            len_queue-=1;machine_type_list.remove(machine_type)
            Ep2=float(E_list[0])##6分子に向けてep作成　ep1:b ep2:a
            E = Ep2##エネルギーの値も変える
            df_E.loc[idx, ['E_p2','E','status']] = [Ep2,E,'Done']
    ...
    if isAvailable:
        ...
        if len(params_dict)!=0:#終わりがまだ見えないなら
            ...
            if not(alreadyCalculated):
                ...
                df_newline = pd.Series({**params_dict,'E':0.,'E_p2':0.,'machine_type':machine_type,'status':'InProgress','file_name':file_name})
