...
from make_ac_qe import exec_gjf##計算した点のxyzfileを出す
...
def main_process(args):
    ...
    auto_csv_path = os.path.join(auto_dir,'step1.csv')
    if not os.path.exists(auto_csv_path):        
        df_E = pd.DataFrame(columns = ['a','b','theta','E','E_p1','E_p2','E_t1','E_t2','E_t3','E_t4','machine_type','status','file_name'])##いじる ,'R3','R4','phi1','phi2'
...
def listen(auto_dir,monomer_name,num_nodes,isTest):##args自体を引数に取るか中身をばらして取るかの違い
    ...
    for idx,row in zip(df_queue.index,df_queue.values):
        ...
        if E_len !=30:##get Eの長さは計算した分子の数
            ...
            E = (Et1+Et2+Et3+Et4)+(Ep1+Ep2)##エネルギーの値も変える
