...
from make_pbebj_p1_3 import exec_gjf##計算した点のxyzfileを出す
...
def init_process(args):
    ...
    def get_init_para_csv(auto_dir,monomer_name):
        ...
        init_para_list = []
        A1 = 0; A2 = 0
        theta_list =[58.0,58.5,59.0,59.5,60.5,61.0,61.5,62.0] #[27.0,29.0,59.0,61.0] #+ [25.5,26.0,26.5,27.0,27.5]##全体を掃く＋細かいところ
...
def listen(auto_dir,monomer_name,num_nodes,isTest):##args自体を引数に取るか中身をばらして取るかの違い
    ...
    for idx,row in zip(df_queue.index,df_queue.values):
        ...
        if E_len !=32:##get Eの長さは計算した分子の数
            ...
            Ep1=float(E_list[0]);Ep2=float(E_list[1]);Et1=float(E_list[2]);Et2=float(E_list[3]);Et3=float(E_list[4]);Et4=float(E_list[5])##6分子に向けてep作成　ep1:b ep2:a
            Em1 = get_E_mono_1(log_filepath)
            Em2 = get_E_mono_2(log_filepath)
            E = (Et1+Et2+Et3+Et4)+(Ep1+Ep2)+Em1+Em2##エネルギーの値も変える
            df_E.loc[idx, ['E_t1','E_t2','E_t3','E_t4','E_p1','E_p2','E_m1','E_m2','E','status']] = [Et1,Et2,Et3,Et4,Ep1,Ep2,Em1,Em2,E,'Done']
    ...
    if isAvailable:
        ...
        if len(params_dict)!=0:#終わりがまだ見えないなら
            ...
            if not(alreadyCalculated):
                ...
                df_newline = pd.Series({**params_dict,'E':0.,'E_p1':0.,'E_p2':0.,'E_t1':0.,'E_t2':0.,'E_t3':0.,'E_t4':0.,'E_m1':0.,'E_m2':0.,'machine_type':machine_type,'status':'InProgress','file_name':file_name})
...
def get_params_dict(auto_dir, num_nodes):
    ...
    fixed_param_keys = ['theta','R3','R4','a','b']
    opt_param_keys = ['phi1','phi2']
...
def get_opt_params_dict(df_cur, init_params_dict,fixed_params_dict):
    ...
    a = init_params_dict['a']; b = init_params_dict['b']
    theta = init_params_dict['theta']
    phi2_init_prev = init_params_dict['phi2'];phi1_init_prev = init_params_dict['phi1']
    while True:
        E_list=[];ab_list=[]
        for phi1 in [phi1_init_prev-1,phi1_init_prev,phi1_init_prev+1]:
            for phi2 in [phi2_init_prev-1,phi2_init_prev,phi2_init_prev+1]:
                a = np.round(a,1);b = np.round(b,1)
                phi1=np.round(phi1);phi2=np.round(phi2)
                df_val_ab = df_val[(df_val['a']==a)&(df_val['b']==b)&(df_val['theta']==theta)&(df_val['phi2']==phi2)&(df_val['phi1']==phi1)&(df_val['status']=='Done')]
                if len(df_val_ab)==0:
                    return False,{'phi1':phi1,'phi2':phi2}
                ab_list.append([phi1,phi2]);E_list.append(df_val_ab['E'].values[0])
        phi1_init,phi2_init = ab_list[np.argmin(np.array(E_list))]
        if phi1_init==phi1_init_prev and phi2_init==phi2_init_prev:
            return True,{'a':phi1_init,'b':phi2_init}
        else:
            phi1_init_prev=phi1_init;phi2_init_prev=phi2_init
