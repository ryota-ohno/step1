...
from make_pbebj_p1 import exec_gjf##計算した点のxyzfileを出す
...
def main_process(args):
    ...
    auto_csv_path = os.path.join(auto_dir,'step1.csv')
    if not os.path.exists(auto_csv_path):        
        df_E = pd.DataFrame(columns = ['a','b','theta','A1','A2','phi1','phi2','E','E_p1','E_p2','E_t1','E_t2','E_m1','E_m2','machine_type','status','file_name'])##いじる
...
def listen(auto_dir,monomer_name,num_nodes,isTest):##args自体を引数に取るか中身をばらして取るかの違い
    ...
    for idx,row in zip(df_queue.index,df_queue.values):
        ...
        if len(E_list)!=4:##get Eの長さは計算した分子の数
            ...
            Ep1=float(E_list[0]);Ep2=float(E_list[1]);Et1=float(E_list[2]);Et2=float(E_list[3])##6分子に向けてep作成　ep1:b ep2:a
            Em1 = get_E_mono_1(log_filepath)
            Em2 = get_E_mono_2(log_filepath)
            E = 2*(Et1+Et2)+(Ep1+Ep2)+Em1+Em2##エネルギーの値も変える
            df_E.loc[idx, ['E_t1','E_t2','E_p1','E_p2','E_m1','E_m2','E','status']] = [Et1,Et2,Ep1,Ep2,Em1,Em2,E,'Done']
    ...
    if isAvailable:
        ...
        if len(params_dict)!=0:#終わりがまだ見えないなら
            ...
            if not(alreadyCalculated):
                ...
                df_newline = pd.Series({**params_dict,'E':0.,'E_p1':0.,'E_p2':0.,'E_t1':0.,'E_t2':0.,'E_m1':0.,'E_m2':0.,'machine_type':machine_type,'status':'InProgress','file_name':file_name})
