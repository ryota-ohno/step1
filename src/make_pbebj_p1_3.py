...
def get_monomer_xyzR(monomer_name,Ta,Tb,Tc,A1,A2,A3,phi=0.0,isFF=False):
    ...
    df_mono=pd.read_csv('~/Working/step1/{}/assets/monomer_3.csv'.format(monomer_name))
...
def get_monomer_xyzR_(monomer_name,Ta,Tb,Tc,A1,A2,A3,phi=0.0,isFF=False):
    T_vec = np.array([Ta,Tb,Tc])
    df_mono=pd.read_csv('~/Working/step1/{}/assets/monomer_4.csv'.format(monomer_name))
    atoms_array_xyzR=df_mono[['X','Y','Z','R']].values
...
def get_xyzR_lines1(xyzR_array,machine_type,file_description):
    if machine_type==1:
        mp_num = 40
    elif machine_type==2:
        mp_num = 52
    lines = [     
        '%mem=15GB\n',
        '%nproc={}\n'.format(mp_num),
        '#P TEST pbepbe/6-311G** EmpiricalDispersion=GD3BJ \n',
        '\n',
        file_description+'\n',
        '\n',
        '0 1\n'
...
def make_gjf_xyz(auto_dir,monomer_name,params_dict,machine_type,isInterlayer):
    ...
    R5=2*R3-R4
    phi1 = params_dict.get('phi1',0.0); phi2 = -params_dict.get('phi2',0.0)##入力ファイルにおけるphi1とphi2の符号は一致させる
    print(phi1, phi2)
    
    monomer_array_i1 = get_monomer_xyzR(monomer_name,0,0,0,A1,A2,A3, phi1)
    monomer_array_i2 = get_monomer_xyzR_(monomer_name,0,0,0,-A1,A2,-A3, phi2)
    if b_>a_:
        monomer_array_p1 = get_monomer_xyzR(monomer_name,a_,0,R5,A1,A2,A3, phi1)
        monomer_array_p2 = get_monomer_xyzR(monomer_name,a_,0,R5,-A1,A2,-A3, phi2)
        ...
        monomer_array_p2 = get_monomer_xyzR(monomer_name,0,b_,R4,-A1,A2,-A3, phi2)
    ...
    monomer_array_t1 = get_monomer_xyzR_(monomer_name,a_/2,b_/2,R3,-A1,A2,-A3, phi2)
    monomer_array_t2 = get_monomer_xyzR_(monomer_name,a_/2,-b_/2,R3-R4,-A1,A2,-A3, phi2)
    monomer_array_t3 = get_monomer_xyzR_(monomer_name,-a_/2,-b_/2,-R3,-A1,A2,-A3, phi2)
    monomer_array_t4 = get_monomer_xyzR_(monomer_name,-a_/2,b_/2,-R3+R4,-A1,A2,-A3, phi2)
    ...
    line_list_monomer_i1 = get_xyzR_lines1(monomer_array_i1,machine_type,file_description+'_i')
    line_list_monomer_i2 = get_xyzR_lines1(monomer_array_i2,machine_type,file_description+'_i')
    ...
    if monomer_name in MONOMER_LIST and not(isInterlayer):
        ...
        gij_xyz_lines = ['$ RunGauss\n'] + line_list_dimer_p1 + ['\n\n--Link1--\n'] + line_list_dimer_p2 + ['\n\n--Link1--\n'] + line_list_dimer_t1 + ['\n\n--Link1--\n'] + line_list_dimer_t2 + ['\n\n--Link1--\n'] + line_list_dimer_t3 + ['\n\n--Link1--\n'] + line_list_dimer_t4 + ['\n\n--Link1--\n'] + line_list_monomer_i1 + ['\n\n--Link1--\n'] + line_list_monomer_i2 + ['\n\n\n']#+ ['\n\n--Link1--\n'] + line_list_dimer_t3 + ['\n\n--Link1--\n'] + line_list_dimer_t4 + ['\n\n\n']
