...
def main_process(args):
    ...
    auto_csv_path = os.path.join(auto_dir,'step1.csv')
    if not os.path.exists(auto_csv_path):        
        df_E = pd.DataFrame(columns = ['a','b','theta','A1','A2','phi1','E','E_p','E_t1','E_t2','machine_type','status','file_name'])##いじる
...
def listen(auto_dir,monomer_name,num_nodes,isTest):##args自体を引数に取るか中身をばらして取るかの違い
    ...
    for idx,row in zip(df_queue.index,df_queue.values):
        ...
        if len(E_list)!=3:##get Eの長さは計算した分子の数
            ...
            Ep=float(E_list[0]);Et1=float(E_list[1]);Et2=float(E_list[2])##6分子に向けてep作成　ep1:b ep2:a
            E = 2*(Et1+Et2)+2*Ep##エネルギーの値も変える
...
def get_params_dict(auto_dir, num_nodes):
    ...
    fixed_param_keys = ['a','A1','A2','phi1']
    opt_param_keys = ['theta','b']
...
def get_opt_params_dict(df_cur, init_params_dict,fixed_params_dict):
    ...
    a = init_params_dict['a']
    b_init_prev = init_params_dict['b']; theta_init_prev = init_params_dict['theta']
    A1 = init_params_dict['A1'];phi1 = init_params_dict['phi1']
    while True:
        E_list=[];ab_list=[]
        for theta in [theta_init_prev-0.5,theta_init_prev,theta_init_prev+0.5]:
            for b in [b_init_prev-0.1,b_init_prev,b_init_prev+0.1]:
                ...
                df_val_ab = df_val[(df_val['a']==a)&(df_val['b']==b)&(df_val['theta']==theta)&(df_val['A1']==A1)&(df_val['phi1']==phi1)&(df_val['status']=='Done')]
                if len(df_val_ab)==0:
                    return False,{'theta':theta,'b':b}
                ab_list.append([theta,b]);E_list.append(df_val_ab['E'].values[0])
        theta_init,b_init = ab_list[np.argmin(np.array(E_list))]
        if theta_init==theta_init_prev and b_init==b_init_prev:
            return True,{'theta':theta_init,'b':b_init}
        else:
            theta_init_prev=theta_init;b_init_prev=b_init
