...
from make_phib import exec_gjf##計算した点のxyzfileを出す
...
def main_process(args):
    ...
    auto_csv_path = os.path.join(auto_dir,'step1.csv')
    if not os.path.exists(auto_csv_path):        
        df_E = pd.DataFrame(columns = ['a','b','theta','A1','A2','phi1','phib','E','E_p','E_t1','E_t2','machine_type','status','file_name'])##いじる
...
def get_params_dict(auto_dir, num_nodes):
    ...
    fixed_param_keys = ['theta','A1','A2','phi1','phib']
    opt_param_keys = ['a','b']
...
def get_opt_params_dict(df_cur, init_params_dict,fixed_params_dict):
    ...
    A1 = init_params_dict['A1'];phi1 = init_params_dict['phi1'];phib = init_params_dict['phib']
    while True:
        E_list=[];ab_list=[]
        for a in [a_init_prev-0.1,a_init_prev,a_init_prev+0.1]:
            for b in [b_init_prev-0.1,b_init_prev,b_init_prev+0.1]:
                ...
                df_val_ab = df_val[(df_val['a']==a)&(df_val['b']==b)&(df_val['theta']==theta)&(df_val['A1']==A1)&(df_val['phi1']==phi1)&(df_val['phib']==phib)&(df_val['status']=='Done')]
                if len(df_val_ab)==0:
