...
def main_process(args):
    ...
    auto_csv_path = os.path.join(auto_dir,'step1.csv')
    if not os.path.exists(auto_csv_path):        
        df_E = pd.DataFrame(columns = ['a','b','theta','E','E_p1','E_p2','E_t','machine_type','status','file_name'])##いじる
...
def listen(auto_dir,monomer_name,num_nodes,isTest):##args自体を引数に取るか中身をばらして取るかの違い
    ...
    for idx,row in zip(df_queue.index,df_queue.values):
        ...
        if len(E_list)!=3:##get Eの長さは計算した分子の数
            ...
            Et=float(E_list[0]);Ep1=float(E_list[1]);Ep2=float(E_list[2])##8分子に向けてep1,ep2作成　ep1:b ep2:a
            E = 4*Et+2*(Ep1+Ep2)##エネルギーの値も変える
    ...
    if isAvailable:
        ...
        if len(params_dict)!=0:#終わりがまだ見えないなら
            ...
            if not(alreadyCalculated):
                file_name = exec_gjf(auto_dir, monomer_name, {**params_dict,'cx':0,'cy':0,'cz':0,'A1':0.,'A2':0.}, machine_type,isTest=isTest)##計算を実行並びにxyzファイルの出力
...
def get_params_dict(auto_dir, num_nodes):
    ...
    fixed_param_keys = ['a','b']
    opt_param_keys = ['theta']
...
def get_opt_params_dict(df_cur, init_params_dict,fixed_params_dict):
    ...
    a = init_params_dict['a']; b = init_params_dict['b']; theta_init_prev = init_params_dict['theta']
    while True:
        E_list=[];theta_list=[]
        for theta in [theta_init_prev-5,theta_init_prev,theta_init_prev+5]:
            a = np.round(a,1);b = np.round(b,1);theta=int(theta)
            df_val_ab = df_val[(df_val['a']==a)&(df_val['b']==b)&(df_val['theta']==theta)&(df_val['status']=='Done')]
            if len(df_val_ab)==0:
                return False,{'theta':theta}
            theta_list.append(theta);E_list.append(df_val_ab['E'].values[0])
        theta_init = theta_list[np.argmin(np.array(E_list))]
        if theta_init==theta_init_prev:
            return True,{'theta':theta_init}
        else:
